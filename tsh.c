...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        ...
        // Child
        if ((pid = safe_fork()) == 0) {
            safe_setpgid(0, 0);                             // set child's group to a new process group (this is identical to the child's PID)
...
void do_bgfg(char **argv) 
{
    ...
    if(strcmp(argv[0], "bg") == 0) {
        ...
        safe_kill(-givenjob->pid, SIGCONT);                              // Send SIGCONT signal to entire group of the given job
    } else {
...
void sigint_handler(int sig) 
{
    // be sure to send SIGINT and SIGTSTP signals to the entire foreground process group, using ”-pid” instead of ”pid” in the argument to the kill function. The sdriver.pl program tests for this error.
    /* ########################### TO DO ########################### */
...
void sigtstp_handler(int sig) 
{
    // be sure to send SIGINT and SIGTSTP signals to the en- tire foreground process group, using ”-pid” instead of ”pid” in the argument to the kill function. The sdriver.pl program tests for this error.
    /* ########################### TO DO ########################### */
